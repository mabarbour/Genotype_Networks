# Matt Barbour has added an offset parameter to allow for estimation of density.

rpt.poisGLMM.multi.offset = function(y, offset.trans, groups, link=c("log", "sqrt"), CI=0.95, nboot=1000, npermut=1000) {
  # initial checks
  ...
  # functions
  pqlglmm.pois.model <- function(y, offset.trans, groups, link, returnR=TRUE) {
  ...
  R   <- pqlglmm.pois.model(y, offset.trans, groups, link)
  
  #browser()
  # confidence interval estimation by parametric bootstrapping
  bootstr <- function(y, offset.trans, groups, k, N, beta0, var.a, omega, link) {
    ...
    pqlglmm.pois.model(y.boot, offset.trans, groups, link) 
  }
  if(nboot > 0) {
    mod.ests <- pqlglmm.pois.model(y, offset.trans, groups, link, returnR=FALSE)
    R.boot   <- replicate(nboot, bootstr(y, offset.trans, groups, k, N, mod.ests$beta0, mod.ests$var.a, mod.ests$omega, link), simplify=TRUE)
  ...
  permut   <- function(y, offset.trans, groups, N, link) {
    samp <- sample(1:N, N)
    pqlglmm.pois.model(y, offset.trans, groups[samp], link) 
  }
  if(npermut > 1) {
    R.permut <- replicate(npermut-1, permut(y, offset.trans, groups, N, link), simplify=TRUE)
