...
#There were too few data points to reliably test for differences in mid-Stem gall diameter (Rabdophaga salicisbattatus), but we used weighted linear models to test for differences in gall size among willow genotypes. We weighted models by the number of galls collected from each plant replicate, because a higher number would reflect a more accurate measure of mean gall size for that particular willow.
...
# Tested whether genotype affected gall community composition. It appears that leaf and bud galls are the major determinants of variation in gall abundance, with apical-Stem galls contributing to a lesser extent. Remember that p-values may slightly differ from those reported in the manuscript since this uses a permutation.
anova.gall <- anova.manyglm(manyglm.gall, p.uni = "unadjusted")
anova.gall 

## testing with linear models
gall.lins <- manylm(log(gall.mvabund+1) ~ Genotype, data = full.df)
summary(gall.lins)
anova(gall.lins, p.uni = "unadjusted") # same qualitative results as GLM


## Test for quantitative differences  in gall community composition.
...
AIC.models.galls.traits <- data.frame(Model = 1:14, # progressive order in which models were fit
                                      ...
                                      AIC = c(sum(AIC(mvabund.galls.traits)), 
                                              ...
                                              sum(AIC(mod13)), 
                                              sum(AIC(mod14.null))))
kable(arrange(AIC.models.galls.traits, AIC)[1:5,]) # display all of the models that had lower AICs than the null model (including the null model).
