...
betalink.q = function(w1,w2,bf="jaccard"){
  ...
  allCom[is.na(allCom)]=0
    ...
    #w1=as.data.frame(w1) # former code from Paul Rabie
		w1=as.data.frame(as.table(w1)) # update from Matt Barbour
    w1Ints = w1$Freq
    names(w1Ints) = paste(w1$Var1,w1$Var2,sep="-")
    #w2=as.data.frame(w2) # former code from Paul Rabie
		w2=as.data.frame(as.table(w2)) # update from Matt Barbour
    w2Ints = w2$Freq
