...
Qualitative differences in gall community composition?
```{r vegan galls}
...
table(gall.comm.sub$Genotype)

gall.comm.raupcrick <- raupcrick(gall.comm.sub[ ,-1])

adonis(gall.comm.raupcrick ~ Genotype, gall.comm.sub)
...
ggplot(gall.plot.df, aes(x = Genotype, y = value, color = variable)) + 
  geom_boxplot() + facet_wrap( ~ variable, nrow = 2) + theme_bw() + ylab("Gall density (#/branch)")
```
...
# get predicted weighted linkage density for each genotype
predict.single.geno <- as.data.frame(round(predict.manyglm(manyglm.full, data.frame(Genotype = levels(full.df$Genotype)), type = "response"),1))
predict.single.geno$Number.of.Genotypes <- rep(1, 26)
predict.single.geno$diversity <- diversity(select(predict.single.geno, -Number.of.Genotypes))
predict.single.geno$repID <- 25001:(25001+25)

link.div.df <- rbind.data.frame(link.div.df, predict.single.geno)

boxplot(diversity ~ factor(Number.of.Genotypes), link.div.df, ylab = "Interaction Diversity", xlab = "Number of Genotypes")

link.melt <- as.data.frame(link.div.df) %>%
  select(-diversity) %>%
  melt(id.vars = c("repID", "Number.of.Genotypes"))
...
single.web.list <- cast(filter(link.melt.df, Number.of.Genotypes == 1), gall ~ parasitoid | repID, mean)
...
single.web.list.measures <- c()
single.genotype.number <- c()
for(i in 1:dim(web.list)){
  genotype.number[i] <- names(single.web.list[[i]][1])
  tmp.web <- single.web.list[[i]][[1]]
  rownames(tmp.web) <- tmp.web$gall
  tmp.web <- as.matrix.data.frame(tmp.web[ ,-1])
  web.list.measures[i] <- networklevel(tmp.web, index = "linkage density", weighted = TRUE)
}
...
write.csv(web.measures.df, "web.measures.df.csv")

ggplot(web.measures.df, 
       aes(x = Number.of.Genotypes, y = Weighted.Linkage.Density)) + 
  ...
  xlab("Genetic variation (# of genotypes)") + ylab("Weighted linkage density") +
  scale_x_continuous(limits = c(0,26), breaks = 1:26) +
  theme_bw() + 
...
```{r complementarity}
net.pred.geno <- as.data.frame(round(predict.manyglm(manyglm.full, 
                                       newdata = data.frame(Genotype = levels(full.df$Genotype)), 
                                       type = "response"), 1)) %>% # round to avoid pseudoaccuracy 
  mutate(Genotype = levels(full.df$Genotype)) %>%
  melt(id.vars = "Genotype")

net.split <- colsplit(net.pred.geno$variable, pattern = "_", names = c("gall","parasitoid"))
net.pred.geno.df <- cbind(net.pred.geno, net.split)

net.pred.full.df <- net.pred.geno.df %>%
  group_by(variable, gall, parasitoid) %>%
  summarise(value = mean(value)) 

net.list <- cast(net.pred.geno.df, gall ~ parasitoid | Genotype, mean)
net.list.noU <- net.list[-21]

net.full <- cast(net.pred.full.df, gall ~ parasitoid, mean)
rownames(net.full) <- net.full$gall
net.full <- as.matrix.data.frame(net.full[ ,-1]) # remove gall variable
net.full.measure <- networklevel(net.full, index = c("links per species", "linkage density", "Shannon diversity"), weighted = TRUE) # note that the weighted linkage density here is higher than from my simulation with the 26 genotypes...
...
net.list.measures <- c()
genotypes <- c()
for(i in 1:dim(net.list.noU)){
  genotypes[i] <- names(net.list.noU)[i]
  tmp.web <- net.list.noU[[i]]
  rownames(tmp.web) <- tmp.web$gall
  nets[[i]] <- as.matrix.data.frame(tmp.web[ ,-1])
  net.list.measures[i] <- networklevel(nets[[i]], index = "linkage density", weighted = TRUE)
}
mean(net.list.measures)
genotypes
net.list.measures.df <- cbind.data.frame(genotypes, net.list.measures)
networklevel(nets[[25]], index = c("links per species", "linkage density", "Shannon diversity"), weighted = TRUE)

qplot(net.list.measures.df, x = factor(1), y = net.list.measures, fill = genotypes, , color = "black", geom = "bar", stat = "identity")
sum(net.list.measures.df$net.list.measures)

ph <- matrix(c(1,1,1), nrow = 1)
networklevel(ph, index = c("links per species", "linkage density", "Shannon diversity", "vulnerability", "generality"), weighted = TRUE)
mean(c(3,1,3,1))
```
...
plot(MASS::glm.nb(vLG_Platy ~ vLG_abund + vLG.height.mean, full.predictors))
```
